...
export class AuthHandler extends BaseHandler {
    ...
    public get type(): string { return "AUTH" }

    constructor(private auth: AuthService, private handlerRegistry: HandlerRegistry) {
        super();
    }
    ...
    public handle(client: Client, packet: Packet): boolean {
        ...
        if (token && this.auth.redeemToken(token)) {
            this.logger.debug(`Client (${client.id}) successfully claimed token: ${token}`);
            client.clearTimeout();
