...
type ParamReturn<T extends ParamTypes> =
    T extends ParamTypes.object ? object : (
        T extends ParamTypes.boolean ? boolean : (
            T extends ParamTypes.number ? number : (
                T extends ParamTypes.string ? string : never)));
...
export class Packet {
    ...
    public getParameter<T extends ParamTypes>(index: number, val: T): ParamReturn<T> {
